...
public class CLexer
{
    ...
    public CLexer(string src, DiagnosticBag? diagnostics = null)
    {
        _src = src;
        _diagnostics = diagnostics ?? new DiagnosticBag();
        
        // Simple macro expansion for EXTERN_FN
        _src = ExpandExternMacros(_src);
    }

    private string ExpandExternMacros(string source)
    {
        // Expand EXTERN_FN(ns, ret, name, ...) to: extern ret ns##__##name(...)
        // Pattern: EXTERN_FN(OCRuntime__PixelBindings, void, FillRect, int64_t x, ...)
        var result = source;
        
        var pattern = @"EXTERN_FN\s*\(\s*([^,]+)\s*,\s*([^,]+)\s*,\s*([^,]+)\s*,\s*([^)]+)\)";
        result = System.Text.RegularExpressions.Regex.Replace(result, pattern, m =>
        {
            var ns = m.Groups[1].Value.Trim();
            var ret = m.Groups[2].Value.Trim();
            var name = m.Groups[3].Value.Trim();
            var args = m.Groups[4].Value.Trim();
            
            // Replace int64_t with long for C compatibility
            args = args.Replace("int64_t", "long");
            
            return $"extern {ret} {ns}__{name}({args})";
        });
    ...
    private void SkipPreprocessorDirective()
    {
    ...
    private CToken LexNumber()
    {
        ...
        if (Current == '0' && (Peek() == 'x' || Peek() == 'X'))
        {
            Advance();
            Advance();
            while (!Eof && (char.IsDigit(Current) || "abcdefABCDEF".Contains(Current)))
                Advance();
        }
        else
        {
        ...
        // Suffixes (f, l, u, ll, ul, etc.)
        while (!Eof && (char.IsLetter(Current) || Current == '_'))
            Advance();
    ...
    private CToken LexString()
    {
        ...
        while (!Eof && Current != '"')
        {
            if (Current == '\\')
            {
                ...
                if (!Eof)
                {
                    ...
                    if (escapedChar == '?')
                    {
                        _diagnostics.AddWarning(DiagnosticCodes.InvalidEscapeSequence,
        ...
        if (Eof)
        {
            _diagnostics.AddError(DiagnosticCodes.UnterminatedString,
