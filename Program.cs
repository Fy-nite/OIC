...
// If the user provided an output file but didn't explicitly select a format,
// default to IR format so `-o <file>` produces an IR file by default.
if (!string.IsNullOrEmpty(cliArgs.OutputFile) && (cliArgs.Format == "fob" || string.IsNullOrEmpty(cliArgs.Format)))
{
    cliArgs.Format = "ir";
}
...
try
{
    ...
    var outputDisplay = !string.IsNullOrEmpty(cliArgs.OutputFile) ? cliArgs.OutputFile : (cliArgs.OutputDirectory ?? "output");
    Console.WriteLine($"OIC: {Path.GetFileName(cliArgs.InputFile)}  Format={cliArgs.Format.ToUpper()}  Output={outputDisplay}");

    var diagnostics = new DiagnosticBag(cliArgs.InputFile);
    ...
    {
        diagnostics.PrintAll(useColor: !cliArgs.Quiet);
        Console.WriteLine();
    }
    ...
    {
        Console.Error.WriteLine($"Compilation failed with {diagnostics.ErrorCount} error(s).");
        Environment.Exit(1);
    }
    
    // Show summary
    if (diagnostics.WarningCount > 0 && !cliArgs.Quiet)
    {
        Console.WriteLine($"Compilation completed with {diagnostics.WarningCount} warning(s).\n");
    }
    ...
    else
    {
        ...
        {
            // Use module name when available for default output filename
            var moduleName = (module?.GetType().GetProperty("Name")?.GetValue(module) as string) ?? "module";
            var ext = cliArgs.Format switch
            {
                "json" => "json",
                "text" => "txt",
                "ir" => "ir",
            ...
            outputFile = Path.Combine(outputDir, moduleName + "." + ext);
        }
...
Format Descriptions:
  ...
  bson      Binary JSON (compact)
    fob       Finite Open Bytecode (binary)
...
class CommandLineArgs
{
    ...
    public string Format { get; set; } = "ir";
