...
public class CCompiler
{
    ...
    private Dictionary<string, (string typeName, string methodName)> _externMethodMap;
    private readonly DiagnosticBag _diagnostics;

    public DiagnosticBag Diagnostics => _diagnostics;

    public CCompiler(DiagnosticBag? diagnostics = null, string? moduleName = null)
    {
        _typeMap = new Dictionary<string, TypeReference>();
        _typedefs = new Dictionary<string, TypeReference>();
        _externMethodMap = new Dictionary<string, (string typeName, string methodName)>();
    ...
    {
        // If no module name was provided, look for a MODULE_METADATA global initializer
        string moduleNameToUse = _moduleName ?? "CProgram";
        foreach (var decl in program.Declarations)
        {
            if (decl is CDeclarationList declList)
            {
                foreach (var init in declList.Declarators)
                {
                    var dname = init.Declarator.Name;
                    if (dname == "MODULE_METADATA" && init.Initializer is CInitializerList list)
                    {
                        // Expect first item to be module name (string literal)
                        if (list.Items.Count > 0 && list.Items[0] is CStringLiteral nameLit)
                        {
                            moduleNameToUse = nameLit.Value;
                        }
                    }
                }
            }
        ...
        _moduleBuilder = new IRBuilder(moduleNameToUse);
    ...
    private void CompileFunction(ClassBuilder classBuilder, CFunctionDeclaration funcDecl)
    {
        ...
        // Support special names that encode an external namespace using "__"
        // Example: OCRuntime__PixelBindings__FillRect -> type: OCRuntime.PixelBindings, name: FillRect
        string originalName = funcDecl.Name;
        string methodName = originalName;
        if (!string.IsNullOrEmpty(originalName) && originalName.Contains("__"))
        {
            var parts = originalName.Split(new string[] { "__" }, System.StringSplitOptions.None);
            if (parts.Length >= 2)
            {
                methodName = parts[^1];
                var typeName = string.Join('.', parts.Take(parts.Length - 1));
                // Map both the simple method name and the full symbol name to the external type + method
                _externMethodMap[methodName] = (typeName, methodName);
                _externMethodMap[originalName] = (typeName, methodName);
            }
        }

        // Map C's `main` to a static `Main` method (capitalized)
        if (methodName == "main")
        {
            methodName = "Main";
        }
        ...
        _currentMethodBuilder = methodBuilder;
    ...
    private void CompileBlock(InstructionBuilder body, CCompoundStatement stmt)
    {
        // Add local declarations to current method
        foreach (var decl in stmt.Declarations)
        {
            if (decl is CDeclarationList declList)
            {
                ...
                if (_currentMethodBuilder != null)
                {
                    ...
                    {
                        var name = declarator.Declarator.Name ?? "local";
                        _currentMethodBuilder.Local(name, type);
                    }
                }
            }
        }

        // Compile statements into provided instruction builder
        foreach (var statement in stmt.Statements)
        {
            CompileStatement(body, statement);
        }
    ...
    private void CompileStatement(InstructionBuilder body, CStatement statement)
    {
        switch (statement)
        {
            ...
            case CIfStatement ifStmt:
                // If the condition is a simple binary comparison, emit left/right
                // onto the stack and use a Binary condition (no compare instruction).
                if (ifStmt.Condition is CBinaryOp condBin && IsComparisonOp(condBin.Op))
                {
                    CompileExpression(body, condBin.Left);
                    CompileExpression(body, condBin.Right);
                    var comp = MapComparisonOp(condBin.Op);
                    body.If(Condition.Binary(comp), thenBuilder => {
                        CompileStatement(thenBuilder, ifStmt.ThenBranch);
                    }, elseBuilder => {
                        if (ifStmt.ElseBranch != null)
                            CompileStatement(elseBuilder, ifStmt.ElseBranch);
                    });
                }
                else
                {
                    // Fallback: compile expression and use stack condition
                    CompileExpression(body, ifStmt.Condition);
                    body.If(Condition.Stack(), thenBuilder => {
                        CompileStatement(thenBuilder, ifStmt.ThenBranch);
                    }, elseBuilder => {
                        if (ifStmt.ElseBranch != null)
                            CompileStatement(elseBuilder, ifStmt.ElseBranch);
            ...
            case CWhileStatement whileStmt:
                // If condition is a simple binary comparison, emit left/right once before loop
                if (whileStmt.Condition is CBinaryOp wbin && IsComparisonOp(wbin.Op))
                {
                    // Emit left/right for initial evaluation
                    CompileExpression(body, wbin.Left);
                    CompileExpression(body, wbin.Right);
                    var comp = MapComparisonOp(wbin.Op);
                    body.While(Condition.Binary(comp), loop => {
                        CompileStatement(loop, whileStmt.Body);
                        // Re-evaluate condition for next iteration
                        CompileExpression(loop, wbin.Left);
                        CompileExpression(loop, wbin.Right);
                    });
                }
                else
                {
                    // Initial condition
                    CompileExpression(body, whileStmt.Condition);
                    body.While(Condition.Stack(), loop => {
                        CompileStatement(loop, whileStmt.Body);
                        // Re-evaluate condition for next iteration
                        CompileExpression(loop, whileStmt.Condition);
                    });
                }
                break;

            case CDoWhileStatement doStmt:
                // Do-while: execute body once, then loop while condition true
                body.LdcI4(1); // initial true to enter loop
                body.While(Condition.Stack(), loop => {
                    CompileStatement(loop, doStmt.Body);
                    CompileExpression(loop, doStmt.Condition);
                });
                break;

            case CForStatement forStmt:
                // Initializer
                if (forStmt.Initializer is CDeclarationList declList)
                {
                    // Add local variables
                    if (_currentMethodBuilder != null)
                    {
                    ...
                    // No initializer expressions handled here
                }
                else if (forStmt.Initializer is CExpression initExpr)
                {
                    CompileExpression(body, initExpr);
                    body.Pop();
                }

                // Condition
                if (forStmt.Condition != null && forStmt.Condition is CBinaryOp fbin && IsComparisonOp(fbin.Op))
                {
                    // initial left/right
                    CompileExpression(body, fbin.Left);
                    CompileExpression(body, fbin.Right);
                    var comp = MapComparisonOp(fbin.Op);

                    body.While(Condition.Binary(comp), loop => {
                        CompileStatement(loop, forStmt.Body);

                        // Increment
                        if (forStmt.Increment != null)
                        {
                            CompileExpression(loop, forStmt.Increment);
                            loop.Pop();
                        }

                        // Re-evaluate left/right
                        CompileExpression(loop, fbin.Left);
                        CompileExpression(loop, fbin.Right);
                    });
                }
                else
                {
                    ...
                    {
                        CompileExpression(body, forStmt.Condition);
                    }
                    else
                    {
                        // no condition => true
                    ...
                    body.While(Condition.Stack(), loop => {
                        ...
                        if (forStmt.Condition != null)
                            CompileExpression(loop, forStmt.Condition);
                        else
                            loop.LdcI4(1);
                    });
                }
            ...
            case CCompoundStatement compStmt:
                // Compile a nested block into the current instruction builder
                CompileBlock(body, compStmt);
                break;
    ...
    private void CompileExpression(InstructionBuilder body, CExpression expr)
    {
        switch (expr)
        {
            ...
            case CIdentifier ident:
                // Load a local variable by name (parameters and locals are defined with names)
                body.Ldloc(ident.Name);
                break;
    ...
    private void CompileFunctionCall(InstructionBuilder body, CFunctionCall call)
    {
        ...
        else
        {
            ...
            {
                _diagnostics.AddWarning(DiagnosticCodes.AssumedInt32,
                    $"Assuming int32 parameter types for call to '{funcName}'");
            }
            // If the function was declared with an extern-style name, emit a call
            // to the external type (e.g., OCRuntime.PixelBindings.FillRect)
            if (_externMethodMap.TryGetValue(funcName, out var mapping))
            {
                var method = new MethodReference(
                    TypeReference.FromName(mapping.typeName),
                    mapping.methodName,
                    TypeReference.Int32,
                    paramTypes
                );
    ...
    {
        // For most operations we evaluate left then right.
        switch (op.Op)
        {
            case "+":
            ...
            case "==":
                ...
                break;
            case "!=":
                // Implement inequality as (left==right) == 0
                CompileExpression(body, op.Left);
                CompileExpression(body, op.Right);
                body.Ceq();
                body.LdcI4(0);
            ...
            case ">":
                ...
                break;
            case "<=":
                // Implement <= as (left==right) || (left<right)
                CompileExpression(body, op.Left);
                CompileExpression(body, op.Right);
                body.Ceq();
                CompileExpression(body, op.Left);
            ...
            case ">=":
                ...
                break;
            default:
                // Fallback: evaluate operands and push 0
                CompileExpression(body, op.Left);
                CompileExpression(body, op.Right);
                body.LdcI4(0);
                break;
    ...
    private static ComparisonOp MapComparisonOp(string op)
    {
        return op switch
        {
            "==" => ComparisonOp.Equal,
...
public class CLanguageCompiler
{
    ...
    public CLanguageCompiler(DiagnosticBag? diagnostics = null)
    {
        _diagnostics = diagnostics ?? new DiagnosticBag();
    }
    ...
    {
        // Detect an optional NAMESPACE define in the source
        string? moduleName = null;
        var m = System.Text.RegularExpressions.Regex.Match(sourceCode, @"^\s*#\s*define\s+NAMESPACE\s+([<\""]?([A-Za-z0-9_.]+)[>\""]?)", System.Text.RegularExpressions.RegexOptions.Multiline);
        if (m.Success)
        {
        ...
        // Lexical analysis
        var lexer = new CLexer(sourceCode, _diagnostics);
        var tokens = lexer.Lex().ToList();

        // Syntax analysis
        var parser = new CParser(tokens, _diagnostics);
        var program = parser.Parse();

        // Code generation
        var compiler = new CCompiler(_diagnostics, moduleName);
