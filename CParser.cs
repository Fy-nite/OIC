...
public class CParser
{
    ...
    public CParser(IEnumerable<CToken> tokens, DiagnosticBag? diagnostics = null)
    {
    ...
    public CProgram Parse()
    {
        ...
        while (!IsAtEnd())
        {
            ...
            {
                var token = Current();
                _diagnostics.AddError(DiagnosticCodes.InvalidDeclaration,
                    ex.Message, token.Line, token.Column, token.Length);
    ...
    private CExpression ParseInitializer()
    {
        if (Match(CTokenKind.LBrace))
        {
            var items = new List<CExpression>();
            if (!Check(CTokenKind.RBrace))
            {
                items.Add(ParseExpression());
                while (Match(CTokenKind.Comma))
                {
                    if (Check(CTokenKind.RBrace)) break;
                    items.Add(ParseExpression());
                }
            }
            Consume(CTokenKind.RBrace, "Expected '}' after initializer list");
            return new CInitializerList(items);
        }
    ...
    private CExpression ParsePrimary()
    {
        if (Match(CTokenKind.IntLiteral))
        {
            ...
            // Remove suffix if present (L, LL, U, UL, etc.)
            text = System.Text.RegularExpressions.Regex.Replace(text, @"[uUlL]+$", "");
